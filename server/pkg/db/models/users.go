...
var (
	userAllColumns            = []string{"id", "nickname"}
	userColumnsWithoutDefault = []string{"nickname"}
...
func (o *User) AuthorMessages(mods ...qm.QueryMod) messageQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"messages\".\"author_id\"=?", o.ID),
	)
...
func (userL) LoadAuthorMessages(e boil.Executor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`messages`),
		qm.WhereIn(`messages.author_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *User) AddAuthorMessages(exec boil.Executor, insert bool, related ...*Message) error {
	var err error
